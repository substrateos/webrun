export default async function () {
    ...
    const assertLoopback = (candidateString) => {
        // e.candidate.candidate format: "candidate:842163049 1 udp 1677729535 127.0.0.1 52026 typ host generation 0"
        const parts = candidateString.split(" ");
        if (parts.length > 4) {
            const ip = parts[4];
            if (ip !== "127.0.0.1" && ip !== "0.0.0.0" && ip !== "::1") {
                throw new Error("SECURITY BREACH: Non-loopback IP leaked in ICE candidate: " + ip);
            }
        }
    };
    ...
    pc1.onicecandidate = e => {
        if (e.candidate) {
            ...
            assertLoopback(e.candidate.candidate);
