// parse.test.ts — Table-driven unit tests for HTML parsing utilities.
//
...
import { parseHtmlScripts, parseHtmlImportMap } from "./parse.ts";
...
import { HTML_FETCH_USER_AGENT } from "./parse.ts";
...
import { readHtmlContent } from "./mod.ts";
...
const networkPermCases: Array<{
    ...
    {
        name: "local file:// .html files bypass network check entirely",
        target: "file:///nonexistent/test.html",
        allowedDomains: [],
        shouldThrow: true,  // Throws because file doesn't exist or lacks read access, not network
        expectedErrorSubstring: "NetworkError", // Deno throws NetworkError for nonexistent file:// URLs
    },
];
...
export async function testHtmlNetworkPermissions(t: any) {
    for (const tc of networkPermCases) {
        await t.run(tc.name, async () => {
            try {
                await readHtmlContent(tc.target, tc.allowedDomains);
                if (tc.shouldThrow) {
