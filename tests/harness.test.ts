...
import { runTestSuite } from "../src/test_harness.ts";
import type { TestContext, HarnessSummary } from "../src/test_harness.ts";
...
async function run(
    ...
    const summary = await runTestSuite(cases, {}, "harness.test.ts", print, filter);
...
export async function testCategory4_Filtering(outerT: TestContext) {
    ...
    await outerT.run("no top-level match → sub-step filter", async (t: TestContext) => {
        ...
        // Parent runs; only "target-step" runs within it.
        t.assert(contains(lines, "target-step"), `Expected target-step in output`);
        t.assert(summary.failed === 0);
    });
}
...
export async function testCategory7_EdgeCases(outerT: TestContext) {
    ...
    await outerT.run("ctx payload is forwarded to test fn", async (t: TestContext) => {
        ...
        await runTestSuite(
            ...
            sentinel, "harness.test.ts", (l) => lines.push(l),
        );
